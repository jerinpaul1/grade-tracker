...
function calculateModuleGrade(ass) {
  let total = 0, wsum = 0;
  ass.forEach(a => { total += (a.mark||0)*(a.weight||0)/100; wsum += a.weight||0; });
  return wsum === 100 ? total : `${total.toFixed(1)} (incomplete)`;
}
...
function render() {
  ...
  data.years.forEach((yr, yi) => {
    ...
    document.getElementById("years").appendChild(yDiv);
  });
...
function importData(evt) {
  ...
  reader.onload = e => {
    try { data = JSON.parse(e.target.result); render(); saveGrades(); }
